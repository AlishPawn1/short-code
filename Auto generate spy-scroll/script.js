$(document).ready(function () {
    const $serviceBoxes = $('.service-left-content .service-box');
    const $sideBarContainer = $('.service-right-content');

    const $ul = $('<ul>', { class: 'side-bar' });

    $serviceBoxes.each(function (index) {
        const id = `spy-scroll-item-service${index + 1}`;
        $(this).attr('id', id);
